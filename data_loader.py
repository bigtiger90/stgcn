...
class PoseDataSetHardDisk(Dataset):
    def __init__(self, data_dir, data_type):
        self.data_dir = data_dir
        self.data_type = data_type
        self.len = json.load(open("{}/meta".format(self.data_dir), "r"))["{}_pair_count".format(data_type)]

    def __getitem__(self, index):
        data = np.load("/{}/{}/pairs/{}/{}.npz".format(self.data_dir, self.data_type, int(index / 10000), index), allow_pickle=True)
        return torch.Tensor(np.array(data["positions_3d"])), torch.Tensor(np.array(data["positions_2d"])), np.array((1)), np.array((1))
...
class PoseBufferHardDisk(Dataset):
    def __init__(self, data_dir, data_type):
        self.data_dir = data_dir
        self.data_type = data_type
        self.len = json.load(open("{}/meta".format(self.data_dir), "r"))["{}_pair_count".format(data_type)]
